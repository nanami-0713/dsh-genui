...
import { cssDeclarations } from './helpers/standalone.ts'

describe('standalone theme', () => {
  it('uses DSH light surface, border, and label tokens', () => {
    const light = cssDeclarations(STANDALONE_THEME_CSS, ':root')
  ...
  it('uses DSH dark surface, border, and label tokens', () => {
    const dark = cssDeclarations(STANDALONE_THEME_CSS, 'body[data-ds-dark-theme]')
